...
func (c *HelmDeployJobCtl) Run(ctx context.Context) {
	...
	if len(c.jobTaskSpec.DeployContents) == 1 && slices.Contains(c.jobTaskSpec.DeployContents, config.DeployImage) {
		// only deploy image
		newEnvService, err = helmDeploySvc.MergedContainerImage(newEnvService, c.jobTaskSpec.DeployImages())
		if err != nil {
			msg := fmt.Sprintf("failed to merge container image, error: %v", err)
			logError(c.job, msg, c.logger)
			return
		}

		finalValuesYaml, envValuesYaml, err = helmDeploySvc.NewGeneMergedValues(newEnvService, productInfo.DefaultValues)
		if err != nil {
		...
		// only deploy config
		finalValuesYaml, _, err = helmDeploySvc.NewGeneMergedValues(newEnvService, productInfo.DefaultValues)
		if err != nil {
			msg := fmt.Sprintf("failed to generate merged values yaml, err: %s", err)
			logError(c.job, msg, c.logger)
			return
	...
	c.jobTaskSpec.UserSuppliedValue = envValuesYaml
	...
	go func(chan bool) {
		if err = kube.DeploySingleHelmRelease(productInfo, newEnvService, tmplSvc, c.jobTaskSpec.Timeout, c.workflowCtx.WorkflowTaskCreatorUsername); err != nil {

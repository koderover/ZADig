...
func EstimatedValues(c *gin.Context) {
	...
	ctx.Resp, ctx.RespErr = service.GenEstimatedValues(projectName, envName, serviceName, service.EstimateValuesScene(c.Query("scene")), arg, updateServiceRevision == "true", production, isHelmChartDeploy == "true", ctx.Logger)
}
